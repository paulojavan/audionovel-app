...
test("listas administrativas e privadas possuem limites explicitos", () => {
  assert.match(source("src", "lib", "admin-user-pagination.ts"), /ADMIN_USERS_PAGE_SIZE\s*=\s*50/);
  assert.match(source("src", "app", "admin", "usuarios", "page.tsx"), /take:\s*ADMIN_USERS_PAGE_SIZE/);
...
test("usuarios administrativos possuem paginacao", () => {
  ...
  assert.match(users, /skip:\s*\(currentPage - 1\) \* ADMIN_USERS_PAGE_SIZE/);
