...
test("renovacao offline serializa upserts em uma unica transacao", () => {
  assert.match(renewRoute, /prisma\.\$transaction\(/);
  assert.match(renewRoute, /prisma\.offlineDownload\.upsert\(/);
  // Promise.all com ate 100 upserts concorrentes ocupava todo o pool (P2037).
