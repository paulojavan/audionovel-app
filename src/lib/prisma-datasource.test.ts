...
test("permite configurar um limite explicito dentro da faixa segura", () => {
  ...
  assert.equal(normalizeConnectionLimit("0"), 5);
  assert.equal(normalizeConnectionLimit("100"), 5);
  assert.equal(normalizeConnectionLimit("invalido"), 5);
});

test("usa cinco conexoes quando URL e ambiente nao configuram o limite", () => {
  const result = getPrismaDatasourceUrl(
