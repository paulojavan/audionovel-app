...
async function getAdminDashboardStats() {
  const [row] = await prisma.$queryRaw<Array<{
    ...
    SELECT
      ...
      (SELECT COALESCE(SUM(
        CASE
          WHEN jsonb_typeof("chapterPartsJson"::jsonb) = 'array' AND jsonb_array_length("chapterPartsJson"::jsonb) > 0
            THEN jsonb_array_length("chapterPartsJson"::jsonb)
          WHEN "positionEnd" IS NOT NULL AND "positionEnd" > "position"
            THEN FLOOR("positionEnd" - "position") + 1
          ELSE 1
        END
      ), 0) FROM "Chapter") AS "chapterCount",
